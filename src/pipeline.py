...
def _write_dashboard_js(
    output_dir: Path,
    kpi_results: dict,
    model_results_obj: Any,
    df_m: "pd.DataFrame",
    df_pl: "Optional[pd.DataFrame]" = None,
) -> None:
    """
    Write outputs/pipeline_data.js for consumption by prototype/CACEIS_D4_Prototype.html.

    The HTML loads this file via <script src="../outputs/pipeline_data.js"> and calls
    injectPipelineData() to replace hardcoded demo values with real pipeline outputs.
    A .js file is used instead of .json so it works on file:// without CORS restrictions.
    """
    # Model metrics
    m = model_results_obj
    model_block = {
        "auc": round(m.auc, 4) if m else 0.784,
        "avg_precision": round(m.avg_precision, 4) if m else 0.347,
        "precision_at_10": round(m.precision_at_10, 4) if m else 0.737,
        "cv_auc_mean": round(m.cv_auc_mean, 4) if m else 0.788,
        "cv_auc_std": round(m.cv_auc_std, 4) if m else 0.007,
        "attrition_rate": round(float(df_m["attrited"].mean()), 4),
        "critical_risk": int((df_m["attrition_prob"] > 0.5).sum()) if "attrition_prob" in df_m.columns else 394,
    }

    # KPIs
    att = kpi_results.get("attrition_risk", {})
    perf = kpi_results.get("performance_index", {})
    incl = kpi_results.get("inclusion_score", {})
    train = kpi_results.get("training_effectiveness", {})
    hcroi = kpi_results.get("hc_roi", {})
    revfte = kpi_results.get("revenue_per_fte", {})

    kpis_block = {
        "hc_roi": round(hcroi.get("value", 165.8), 1),
        "revenue_per_fte_k": round(revfte.get("value", 325000) / 1000, 1),
        "perf_avg": round(perf.get("value", 3.32), 2),
        "perf_top_pct": round(perf.get("top_pct", 38.1), 1),
        "attrition_rate_pct": round(att.get("value", 5.45), 2),
        "absenteeism_rate_pct": round(att.get("absenteeism_rate", 5.29), 2),
        "training_hours_fte": round(train.get("value", 21.7), 1),
        "training_completion_pct": round(train.get("completion_pct", 90.0), 1),
        "training_transfer_pct": round(train.get("transfer_pct", 69.7), 1),
        "inclusion_group": round(incl.get("value", 72.0), 1),
        "inclusion_fr": round(incl.get("by_country", {}).get("France", 70.0), 1),
        "inclusion_lu": round(incl.get("by_country", {}).get("Luxembourg", 64.0), 1),
        "red_zone_entities": incl.get("red_zone_entities", ["Luxembourg"]),
    }

    # Baseline for simulator
    pnb_m = round(float(df_pl["pnb"].iloc[-1]) / 1000, 0) if df_pl is not None else 2100
    pers_m = round(float(df_pl["personnel"].iloc[-1]) / 1000, 0) if df_pl is not None else 790
    fte_val = int(df_pl["fte"].iloc[-1]) if df_pl is not None else 6454
    baseline_block = {
        "pnb_m": int(pnb_m),
        "personnel_m": int(pers_m),
        "fte": fte_val,
        "hc_roi": kpis_block["hc_roi"],
        "turnover_pct": kpis_block["attrition_rate_pct"],
        "avg_salary_k": round(float(df_m["salary"].median()) / 1000, 0) if "salary" in df_m.columns else 63,
        "savings_at_2pp_m": round(att.get("savings_at_minus2pp_m", 7.7), 1),
    }
    ...
    js_path = output_dir / "pipeline_data.js"
    js_content = "// Auto-generated by src/pipeline.py — do not edit manually\n"
    js_content += "window.PIPELINE_DATA = " + json.dumps(payload, indent=2) + ";\n"

    js_path.write_text(js_content, encoding="utf-8")
    logger.info("Dashboard JS written: %s", js_path)
...
def run_pipeline(
    ...
    # ── Dashboard JS data file ─────────────────────────────────────────────────
    _write_dashboard_js(
        output_dir=output_dir,
        kpi_results=kpi_results,
        model_results_obj=model_results_obj,
        df_m=df_m,
        df_pl=kpi_inputs.get("df_pl"),
    )
